...
func TestNotifyErr(t *testing.T) {
	...
	t.Run("when there is error handlers", func(t *testing.T) {
		...
		log := syncLogs.Front()
		assert.Contains(t, log.Value, "github.com/sttk/sabi.Err { reason = github.com/sttk/sabi.FailToDoSomething }-1:err-notify_test.go:218:")
		log = log.Next()
		assert.Contains(t, log.Value, "github.com/sttk/sabi.Err { reason = github.com/sttk/sabi.FailToDoSomething }-2:err-notify_test.go:218:")
		log = log.Next()
		assert.Nil(t, log)
		...
		log = asyncLogs.Front()
		assert.Contains(t, log.Value, "github.com/sttk/sabi.Err { reason = github.com/sttk/sabi.FailToDoSomething }-4:err-notify_test.go:218:")
		log = log.Next()
		assert.Contains(t, log.Value, "github.com/sttk/sabi.Err { reason = github.com/sttk/sabi.FailToDoSomething }-3:err-notify_test.go:218:")
		log = log.Next()
		assert.Nil(t, log)
	})
}
